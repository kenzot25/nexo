...
| `query_service.py` | graph query helpers | graph file path + query input → structured graph query results |
...
| `workspace.py` | `run_workspace_update(...)` | directory → multi-repo graph indexing & registry |
...
- Graph file paths → `validate_graph_path()` (must resolve inside `nexo-out/`)
