...
nexo is a **local development tool**. It runs as a Claude Code skill and optionally as a local MCP stdio server. It makes no network calls during graph analysis - only during `ingest` (explicit URL fetch by the user).
...
| Path traversal in MCP server | Query loading only allows `.json` graphs inside `nexo-out/` or `workspace-nexo-out/`, rejecting arbitrary filesystem paths. |
