...
def test_load_graph_missing_file(tmp_path):
    nexo_dir = tmp_path / "nexo-out"
    nexo_dir.mkdir()
    with pytest.raises(SystemExit):
        _load_graph(str(nexo_dir / "nonexistent.json"))
...
def test_print_stdio_hint_if_interactive(monkeypatch):
    monkeypatch.setattr("sys.stdin", _FakeTty(True))
    stderr = _FakeTty(True)
    monkeypatch.setattr("sys.stderr", stderr)

    _print_stdio_hint_if_interactive()

    assert "waiting for a client" in stderr.getvalue()


def test_print_stdio_hint_if_not_interactive(monkeypatch):
