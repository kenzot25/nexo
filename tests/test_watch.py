...
from nexo.watch import _notify_only, _WATCHED_EXTENSIONS
...
def test_notify_only_creates_flag(tmp_path):
    ...
    flag = tmp_path / "nexo-out" / "needs_update"
    assert flag.exists()
    assert flag.read_text() == "1"

def test_notify_only_creates_flag_dir(tmp_path):
    # nexo-out dir does not exist yet
    assert not (tmp_path / "nexo-out").exists()
    _notify_only(tmp_path)
    assert (tmp_path / "nexo-out").is_dir()
