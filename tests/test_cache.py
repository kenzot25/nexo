"""Tests for nexo/cache.py."""
import pytest
from pathlib import Path
from nexo.cache import file_hash, cache_dir, load_cached, save_cached, cached_files, clear_cache, _body_content
...
def test_clear_cache(tmp_file, cache_root):
    """clear_cache removes all .json files from nexo-out/cache/."""
    save_cached(tmp_file, {"nodes": [], "edges": []}, root=cache_root)
    assert len(list((cache_root / "nexo-out" / "cache").glob("*.json"))) > 0
