...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"nexo requires tree-sitter >= 0.23.0 (Language API v2). "
...
def extract(paths: list[Path], cache_root: Path | None = None) -> dict:
    ...
    Args:
        ...
        cache_root: explicit root for nexo-out/cache/ (overrides the
            inferred common path prefix). Pass Path('.') when running on a
            subdirectory so the cache stays at ./nexo-out/cache/.
    """
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from nexo.detect import _load_nexoignore, _is_ignored
    ignore_root = root if root is not None else target
    patterns = _load_nexoignore(ignore_root)
