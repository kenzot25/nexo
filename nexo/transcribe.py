...
_DEFAULT_MODEL = "base"
_TRANSCRIPTS_DIR = "nexo-out/transcripts"
_FALLBACK_PROMPT = "Use proper punctuation and paragraph breaks."


def _model_name() -> str:
    return os.environ.get("NEXO_WHISPER_MODEL", _DEFAULT_MODEL)
...
def build_whisper_prompt(god_nodes: list[dict]) -> str:
    ...
    domain hint from these labels and passes it via NEXO_WHISPER_PROMPT or
    as initial_prompt — no separate API call needed here.
    """
    if not god_nodes:
        return _FALLBACK_PROMPT

    override = os.environ.get("NEXO_WHISPER_PROMPT")
    if override:
        return override
