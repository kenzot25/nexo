...
_MANIFEST_PATH = "nexo-out/manifest.json"
...
def _load_nexoignore(root: Path) -> list[tuple[Path, str]]:
    """Read .nexoignore from root **and ancestor directories**.
    ...
    the .nexoignore file was found — so patterns written relative to a
    parent directory still work when nexo is run on a subfolder.
    ...
    while True:
        ignore_file = current / ".nexoignore"
...
def _parse_gitignore_line(raw: str) -> str | None:
    r"""Parse one gitignore line, honoring escaped markers.

    - `#` starts a comment unless escaped as `\#`
    - `!` negates unless escaped as `\!`
...
def _load_gitignore(root: Path) -> list[str]:
    """Read .gitignore patterns from the scan root.

    This is intentionally repo-local: workspace mode scans each repository
    independently so each repo's own .gitignore controls what gets skipped.
    """
    ...
    for raw in ignore_file.read_text(encoding="utf-8", errors="ignore").splitlines():
        parsed = _parse_gitignore_line(raw)
        if parsed:
            patterns.append(parsed)
    return patterns


def _gitignore_match(path: Path, root: Path, pattern: str, *, is_dir: bool) -> bool:
    """Return True when a single gitignore pattern matches path.

    Supports the common subset needed for dependency/build directories
    (e.g., node_modules/, dist/, target/, .venv/, *.min.js).
    """
    try:
    ...
    # Slash-less patterns match any path segment (including basename).
    if "/" not in p:
        if fnmatch.fnmatch(path.name, p):
            return True
        return any(fnmatch.fnmatch(part, p) for part in parts)

    # Non-anchored patterns with slash can match anywhere in the relative path.
    return fnmatch.fnmatch(rel, p) or fnmatch.fnmatch(rel, f"**/{p}")


def _is_gitignored(path: Path, root: Path, patterns: list[str], *, is_dir: bool) -> bool:
    """Return True if path is ignored by gitignore patterns.

    Honors basic negation precedence: later patterns override earlier ones.
    """
    if not patterns:
        return False
    ...
    for raw in patterns:
        ...
        if _gitignore_match(path, root, pattern, is_dir=is_dir):
            ignored = not negated
    return ignored


def _is_ignored(path: Path, root: Path, patterns: list[tuple[Path, str]]) -> bool:
    """Return True if path matches any .nexoignore pattern."""
    if not patterns:
        return False
    ...
    for anchor, pattern in patterns:
        ...
        try:
            ...
            pass
        # Also try relative to the anchor dir (the .nexoignore's location),
...
def detect(root: Path, *, follow_symlinks: bool = False, respect_gitignore: bool = True) -> dict:
    ...
    ignore_patterns = _load_nexoignore(root)
    gitignore_patterns = _load_gitignore(root) if respect_gitignore else []

    # Always include nexo-out/memory/ - query results filed back into the graph
    memory_dir = root / "nexo-out" / "memory"
    scan_paths = [root]
        ...
        for dirpath, dirnames, filenames in os.walk(scan_root, followlinks=follow_symlinks):
            ...
            if not in_memory_tree:
                ...
                dirnames[:] = [
                    ...
                    and not _is_gitignored(dp / d, root, gitignore_patterns, is_dir=True)
                ]
    ...
    converted_dir = root / "nexo-out" / "converted"
    ...
    for p in all_files:
        ...
        if _is_ignored(p, root, ignore_patterns):
            continue
        if _is_gitignored(p, root, gitignore_patterns, is_dir=False):
            continue
        if _is_sensitive(p):
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install nexo[office]]")
                continue
    ...
    return {
        ...
        "nexoignore_patterns": len(ignore_patterns),
