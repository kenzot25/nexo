...
from nexo.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS
...
def _rebuild_code(
    ...
    *,
    ...
    try:
        ...
        from nexo.export import to_json, to_obsidian
        ...
        if not code_files:
            print("[nexo watch] No code files found - nothing to rebuild.")
            return False
        ...
        out = out_dir or (watch_path / "nexo-out")
        ...
        if obsidian_sync:
            ...
            print(f"[nexo watch] Obsidian vault updated: {notes} note(s) in {vault_dir}")
        ...
        print(f"[nexo watch] Rebuilt: {G.number_of_nodes()} nodes, "
              f"{G.number_of_edges()} edges, {len(communities)} communities")
        print(f"[nexo watch] graph.json and GRAPH_REPORT.md updated in {out}")
        return True

    except Exception as exc:
        print(f"[nexo watch] Rebuild failed: {exc}")
        return False


def _notify_only(watch_path: Path) -> None:
    """Write a flag file and print a notification (fallback for non-code-only corpora)."""
    flag = watch_path / "nexo-out" / "needs_update"
    flag.parent.mkdir(parents=True, exist_ok=True)
    flag.write_text("1", encoding="utf-8")
    print(f"\n[nexo watch] New or changed files detected in {watch_path}")
    print("[nexo watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[nexo watch] Run `/nexo --update` in Claude Code to update the graph.")
    print(f"[nexo watch] Flag written to {flag}")
...
def watch(
    ...
    *,
    ...
    if not config.stop_requested:
        print(f"[nexo watch] Watching {watch_path.resolve()}")
        print(f"[nexo watch] Code changes rebuild graph automatically.")
        print(f"[nexo watch] Debounce: {config.debounce}s")
    ...
    try:
        while not config.stop_requested:
            ...
            if pending and (time.monotonic() - last_trigger) >= config.debounce:
                ...
                batch = list(changed)
                changed.clear()
                print(f"\n[nexo watch] {len(batch)} file(s) changed")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the nexo graph")
