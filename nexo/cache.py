...
def cache_dir(root: Path = Path(".")) -> Path:
    """Returns nexo-out/cache/ - creates it if needed."""
    d = Path(root) / "nexo-out" / "cache"
...
def load_cached(path: Path, root: Path = Path(".")) -> dict | None:
    ...
    Cache value: stored as nexo-out/cache/{hash}.json
...
def save_cached(path: Path, result: dict, root: Path = Path(".")) -> None:
    ...
    Stores as nexo-out/cache/{hash}.json where hash = SHA256 of current file contents.
...
def clear_cache(root: Path = Path(".")) -> None:
    """Delete all nexo-out/cache/*.json files."""
    d = cache_dir(root)
